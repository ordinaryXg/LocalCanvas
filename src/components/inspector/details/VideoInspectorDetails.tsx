...
export function VideoInspectorDetails({ node, nodes, edges }: Props) {
  ...
  const hasAsset = !!(data.videoSrc || data.videoAssetPath)
  const hasFirstFrame = !!(data.firstFrameSrc || data.firstFrameAssetPath)
  const hasLastFrame = !!(data.lastFrameSrc || data.lastFrameAssetPath)
  const isGenerating = data.isGenerating === true
  const hasError = typeof data.error === 'string' && data.error.length > 0
  const prompt = (data.prompt as string) || ''

  const warnings = useMemo(
    () => collectInboundEdgeWarnings(node.id, nodes, edges, GENERATION_CONSUMED_HANDLES.video),
    [node.id, nodes, edges],
  )
  ...
  if (hasFirstFrame) chips.push({ label: '首帧就绪', tone: 'default' })
  if (hasLastFrame) chips.push({ label: '尾帧就绪', tone: 'default' })

  if (modelId) {
    chips.push({ label: modelNames[modelId] ?? modelId, tone: 'default', onClick: openDrawer })
  }
  if (style) {
    chips.push({ label: style.name, tone: 'default', onClick: openDrawerFocusStyle })
  }

  const summaryLines: (string | null)[] = []
  if (data.fileName) summaryLines.push(`文件 · ${String(data.fileName)}`)
  if (data.duration) summaryLines.push(`时长 · ${data.duration}s`)
  if (sourceLabel) summaryLines.push(`prompt ← ${sourceLabel}`)
  else if (prompt) summaryLines.push(`prompt · ${prompt.length} 字`)

  return (
    <div className="space-y-4">
      <InspectorStatusThumb nodeId={node.id} hasAsset={hasAsset} kind="video" />
