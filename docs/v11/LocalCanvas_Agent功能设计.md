...
**升舱规则**：`targetDurationSec > 15`、出现「分镜/多镜头/品牌/故事/蒙太奇」等信号时，**不得**仅输出三节点 `text-to-video` auto 计划。
...
| 工作流模板开关 | `lc-agent-disabled-templates` | 取代原 `disabled-skills` 命名；兼容读取旧 key |
| 默认 Agent 模式 | `lc-agent-default-mode` | `auto`：有选中→Build，无选中→Plan |
| DAG 检查点 | `lc-agent-checkpoint-enabled` | 是否启用 `checkpoint` 执行模式（默认开） |
| 确认后自动执行 | `lc-agent-auto-run` | 对 `auto` 计划是否立即 DAG（默认开） |
| 能力预检摘要 | 运行时计算 | 按片型展示：尾帧 / Vision / 参考图 / TTS |
| 默认片型轨道 | `lc-agent-default-track` | `auto`：由时长与关键词推断 Lite/Studio |
| 每镜 Take 数 | `lc-agent-takes-per-shot` | 1–3，复杂片默认 2（v12 执行） |
...
| 项 | 说明 |
|----|------|
| Settings Agent Tab | §六全部 |
| 模板注册表 **内部统一** | Skill 与 workflow-templates 同构；对外行为不变 |
| 模板召回 UI | Plan 模式 Top 1–3 卡片 + 采纳（**替代**静默 Skill 命中） |
| 能力预检与跳转 | blocking 警告链到 Settings |
| Agent P1 跑表 | ≥ 80% |
| 文档 | agent-guide 更新 |
...
| `GraphPatch` | 增量改图 + 预览 |
| `executionMode: checkpoint` | DAG 在 script/storyboard 前暂停 |
| 预览「换一个方案」 | 同会话重规划 |
