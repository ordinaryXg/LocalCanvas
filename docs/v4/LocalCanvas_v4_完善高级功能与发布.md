...
> **前置条件**：v3 验收通过  
> **生成日期**：2026-06-04  
> **最后更新**：2026-06-05（版本总览增补 v5/v6；演进方向同步）
...
| 版本 | 核心能力 | 周期 | 文档 |
|------|----------|------|------|
| **v1** | 画布 + 5种节点 + 连线 + 项目存取 | 2 周 | [v1](../../LocalCanvas_v1_画布基础与节点系统.md) |
| **v2** | 模型配置 + 生成器 + 脚本节点 | 2.5 周 | [v2](../../LocalCanvas_v2_模型配置与生成器系统.md) |
| **v3** | 视频合成 + 时间轴 + 项目打磨 | 2.5 周 | [v3](../../LocalCanvas_v3_视频合成与项目打磨.md) |
| **v4** | 高级适配器 + 历史/工作流 + 发布 | 2 周 | 本文档 |
| **v5** | Agent + DAG + 分镜组 + 本地用户系统 | 3.5 周 | [v5](../../LocalCanvas_v5_Agent自动化与分镜增强.md) |
| **v6 客户端** | 合成剪辑台 + 文本双栏 + 模型能力 Registry | 2.5 周 | [v6](../../LocalCanvas_v6_节点体验与能力系统.md) |
| v6 云端扩展 | 独立服务端 + 账号迁移 + 导演台 | 规划 | v5 §十一 |
| **Core 路径** | v1–v4 可发布工具 | **约 9 周** | [开发步骤表](./LocalCanvas_开发步骤表.md) |

---

## 九、v4 之后的演进方向

> **状态更新（2026-06-05）**：下表「v5/v6 处理」列标明各方向在后续版本中的归属。

| 方向 | 说明 | 优先级 | v5/v6 处理 |
|------|------|--------|------------|
| Agent 模式 | 自然语言 → 工作流计划 + 执行 | 高 | ✅ [v5](../../LocalCanvas_v5_Agent自动化与分镜增强.md) |
| 分镜组 | 批量分镜管理 + 宫格预览 | 中 | ✅ v5 |
| Slash 快捷 | 九宫格/二十五宫格布局 | 中 | ✅ v5 |
| 人声分离 | 音频人声/背景音分离 | 中 | ✅ v5 |
| 用户/账号 | 本地注册登录 | — | ✅ v5（本地 SQLite） |
| 模型能力系统 | Registry 驱动画布连线与选模 | 高 | ✅ [v6 客户端](../../LocalCanvas_v6_节点体验与能力系统.md) |
| 合成剪辑台 v2 | ComposeEditor 大面板时间轴 | 中 | ✅ v6 |
| 文本节点 v2 | draft/output 双栏编辑 | 中 | ✅ v6 |
| 导演台 | 3D 场景搭建 + 角色布局 | 高 | ⏭️ v6 云端扩展轨道 |
| 云端服务端 | 账号迁移 + 数据同步 | 高 | ⏭️ v6 云端扩展轨道 |
| 字幕擦除 | 视频字幕 AI 擦除 | 中 | ⏭️ v6 云端扩展（v5 仅 SRT 叠加/烧录） |
| 风格模板库 | 预置视频风格模板 | 低 | 🔶 v5 轻量预设 |
| 多语言 | i18n 支持 | 低 | 🔶 v5 补全；v6 能力文案待做 |
| 移动端预览 | 手机端预览项目 | 低 | ⏭️ v6 云端扩展 |
| 社区分享 | 工作流/模板分享平台 | 低 | ❌ 不在路线图 |
| OpenClaw 集成 | 外部 Agent 框架 | — | ❌ 不在路线图 |
