...
> **前置条件**：v2 验收通过  
> **生成日期**：2026-06-04  
> **最后更新**：2026-06-05（增补 v3→v6 合成剪辑台衔接）
...
| v3 产出 | v4 扩展 |
|---------|---------|
| OpenAI 兼容 + Seedance | Replicate + 自定义 HTTP 适配器 |
...
v3 的 `TimelinePanel` 底部浮层在 v6 升级为 **ComposeEditor 剪辑台**（75% 高面板），时间轴即素材列表，画布与剪辑台分工更清晰。

| v3 产出 | v6 扩展 |
|---------|---------|
| `TimelinePanel` 底部浮层 | `ComposeEditor` 大面板 + 专注模式 |
| 合成节点内片段列表 | 移除；统一在时间轴编辑 |
| 顺序拼接 + 字幕/音频轨 | 保留；增加检查器、裁切边缘拖拽 |
| `compose-service` 导出 | 行为不变；待补 `audioVolume` FFmpeg 滤镜 |

详案：[合成编辑器重设计](./v6/design/LocalCanvas_合成编辑器重设计.md) · 总览：[v6](./LocalCanvas_v6_节点体验与能力系统.md) · [开发步骤表 Phase 6](./LocalCanvas_开发步骤表.md#phase-6节点体验--模型能力系统v6-客户端约-25-周)
