...
> **版本目标**：**处理掉 §一 全部技术债**（50 项中 v9 已关 **7** 项，v10 须清零剩余 **43** 项），不新增大型业务能力  
> **预计周期**：3 周（15 个工作日，5 个 Wave + 验收）  
> **前置条件**：v9 四波已落地；`npm test`（166）与 `npm run build` 通过  
> **生成日期**：2026-06-08（v10 定稿：技术债优先）  
> **立场**：v10 **唯一主线 = 技术债清零**；优化方向不再待定，按本文 Wave 顺序执行直至 §一 无 ⬜/🔶

**图例**：⬜ 未开始　🔶 部分完成（须写清补全标准）　✅ 已完成（含 v9 关闭）
...
| v9 已交付 | v10 必须完成 |
|-----------|--------------|
| 包体 lazy、R2 报告、e2e 5 条、工作台预览、一键断开、死代码一批 | **剩余 43 项债务全部关闭** |
| P1 约 75%（含部分） | P1 **100%**；P0/P2/P3 **100%** 或写入 §六「独立轨道」并移出总表 |

v10 不做新功能扩张；每一项债务须有**代码/测试/文档**证据与验收勾选。

### 0.2 版本性质（已定）

| 维度 | 说明 |
|------|------|
| **主线** | 技术债清零（工程 + 能力 + 壳层 + 测试） |
| **优化方向** | **不选型** — 按 §三 Wave 0→5 顺序还清 |
| **本文档角色** | 唯一待办真相源；新债只追加 §一 |
| **排期原则** | P0 → P1 → P2 → P3；同优先级内按 Wave 表顺序 |

### 0.3 成功标准（v10 收官）

| ID | 指标 | 目标 |
|----|------|------|
| **V10-R1** | 债务清零 | §一 **43 项待办全部 ✅**；总表 50 项无 ⬜/🔶 |
| **V10-R2** | P1 门禁 | P1（23 项）**100%** 关闭 |
| **V10-R3** | 质量 | `npm test` 全绿；`npm run build` 通过；e2e smoke **≥ 6**（含 Legacy） |
| **V10-R4** | 用例 | v5 附录 A P1 **≥ 80%** 通过，跑表记录归档 |
| **V10-R5** | Legacy | 策略落地（**推荐：移除**）+ 死文件删除 + smoke |
| **V10-R6** | 可维护 | 5 个热点大文件完成拆分（§二）或单项验收说明 |

---

## 一、技术债总表（50 项 · 43 项待清零）

> v9 已关 **7 项**（✅：UI-03、CAP-06/07、ENG-01/04/05、QA-01）；下表对待办项给出**具体任务**与**验收标准**。

### 1.1 界面与壳层（14 项 · 待办 13）
...
> 注：统计含 🔶 补全项；收官时 §一 状态列全部为 ✅。

---

## 二、大文件拆分清单（T10-ENG-14）

| 文件 | ~行数 | 拆出模块 | 关联债务 |
|------|-------|----------|----------|
| `ModelSettingsSection.tsx` | ~593 | `ModelList` / `CapabilityDetail` / `ProbePanel` | T10-UI-02 |
| `ImageEditorPanel.tsx` | ~530 | `ImagePreviewColumn` / `ImageParamsColumn` | T10-UI-13 |
| `TextEditorPanel.tsx` | ~416 | `TextDraftPane` / `TextOutputPane` / `ReasoningBlock` | T10-CAP-04 |
| `Canvas.tsx` | ~550 | `useCanvasSlash` / `useCanvasDag` / `useCanvasLegacy` | T10-ENG-02/09 |
| `ComposeEditor.tsx` | ~440 | `useComposeTimeline` hook | T10-CAP-01 |

---

## 三、实施路线图（技术债清零 · 5 Wave）

### Wave 0 — 基线与门禁（2 天）

| 序 | 任务 ID | 内容 |
|----|---------|------|
| 0.1 | T10-QA-05 | v5 P1 跑表，产出缺口清单 |
| 0.2 | T10-ENG-02 | **Legacy 决策：移除**（写入 §五 ADR） |
| 0.3 | — | 建立 v10 进度表（§四），每项指定负责人 |

**出口**：跑表完成；Legacy 策略签字；Wave 1 可开工。

### Wave 1 — P0 清零（3 天）

| 序 | 任务 ID | 内容 |
|----|---------|------|
| 1.1 | T10-CAP-11 | Token 语义层合并 |
| 1.2 | T10-ENG-02 | 删除 Legacy 主路径代码 |
| 1.3 | T10-QA-05 | 按跑表结果修 P0/P1 阻断项 |

**出口**：P0 四项全 ✅；主路径仅 EditorShell。

### Wave 2 — P1 能力 + 工程（4 天）

| 序 | 任务 ID | 内容 |
|----|---------|------|
| 2.1 | T10-CAP-02 | Dock 健康总览 |
| 2.2 | T10-CAP-03 | 能力 Pin / catalog version |
| 2.3 | T10-CAP-04 | reasoning 折叠 UI |
| 2.4 | T10-CAP-05 | 图片多参考端口 |
| 2.5 | T10-EXE-01 | DAG 崩溃恢复弹窗 |
| 2.6 | T10-ENG-03 | Slash/Generator 收敛 |
| 2.7 | T10-ENG-09/10/11 | 性能三板斧 |
| 2.8 | T10-UI-12 | zinc → token |
| 2.9 | T10-QA-02/03 | Legacy smoke（或 N/A）+ auth 单测 |
...
| 序 | 任务 ID | 内容 |
|----|---------|------|
| 3.1 | T10-UI-01~11 | 壳层、a11y、响应式、微反馈（按 §1.1 表） |
| 3.2 | T10-UI-13/14 | 零滚动验收；`/style` 收尾 |
| 3.3 | T10-CAP-01/08/09/10 | 合成淡入淡出 + 适配 + i18n |
| 3.4 | T10-EXE-02~05 | 分离 API + Agent 设置 + account-guide |
| 3.5 | T10-ENG-06/07/12/13/14 | 类型迁移、依赖审计、保存 debounce、缓存、拆分 |
| 3.6 | T10-QA-04/06 | 补单测 + 100 节点归档 |
...
| 序 | 任务 ID | 内容 |
|----|---------|------|
| 4.1 | T10-ENG-08 | main 依赖审计 |
| 4.2 | — | 全表 §一 复核：50 项全 ✅ |
| 4.3 | — | `npm test` + `npm run build` + `npm run test:e2e` |
| 4.4 | — | 更新 v9/v10 文档；CHANGELOG v10 |
...
| 关联 | T10-ENG-02、T10-ENG-03、T10-QA-02、T10-UI-14 |
| 验收 | 主路径 `App.tsx` 仅 `EditorShell`；`npm test` + e2e 绿 |

---

## 六、明确不在 v10 范围（不进入 §一 总表）

| 项 | 说明 |
|----|------|
| 云端服务端 / 数据云同步 | 独立产品轨道 |
| 导演台 3D / 移动端 / 协作 | 未规划 |
| 新供应商人声分离**商业接入** | 若超出现有 IPC 骨架，单独立项（EXE-02 仅接配置+adapter） |
...
```
docs/
  LocalCanvas_v10_项目优化与技术债归集.md   # 本文
...
src/
  layouts/EditorShell.tsx                   # T10-UI-05
  layouts/modes/WorkbenchMode.tsx           # 主工作台（保留）
  components/panels/SettingsPanel.tsx       # T10-UI-02 / EXE-03/04
  components/panels/TextEditorPanel.tsx     # T10-CAP-04
  components/panels/WorkbenchNodePreview.tsx
  components/shell/TopBar.tsx               # T10-UI-06/08
  components/shell/Dock.tsx                 # T10-CAP-02 / UI-07
  components/shell/ModeSwitcher.tsx         # T10-UI-12
  components/agent/AgentCompanion.tsx         # T10-UI-12
  components/canvas/SlashCommandPalette.tsx # T10-UI-04
  components/inspector/ProjectSummary.tsx     # T10-CAP-02 联动
  capabilities/edge-compat.ts               # T10-ENG-13
  hooks/useDagRun.ts                        # T10-EXE-01
  stores/editorShellStore.ts                # T10-UI-01
  styles/index.css                          # T10-CAP-11

删除目标（T10-ENG-02）:
  layouts/LegacyAppLayout.tsx
  layouts/modes/GenerateMode.tsx
  layouts/modes/EditMode.tsx
  components/panels/GeneratorPanel.tsx      # 若 Canvas 不再引用

electron/
  main/services/auth-service.ts             # T10-QA-03
  utility/services/compose-service.ts       # T10-CAP-01

e2e/
  workbench-smoke.spec.ts                   # ✅
  legacy-smoke.spec.ts                      # T10-QA-02 或 N/A

scripts/
  report-bundle-size.mjs                    # T10-ENG-07 扩展
```
...
> v9 让 LocalCanvas **轻、快、顺眼**；v10 让它 **债清、可测、可维护**。

v10 **不做方向选择题** — 只按 §三 Wave 0→4 逐项清零 §一。收官唯一标准：**总表无 ⬜、无 🔶**。

---

*本文档为 v10 唯一待办源；实施进度更新 §四 勾选与 §八 状态，直至全部 ✅。*
