...
- [ ] 应用可启动，显示项目列表页
- [ ] 可创建/打开/删除项目（SQLite 存储）
- [ ] 画布支持无限缩放（0.1x-4x）/平移/小地图导航
- [ ] 5 种节点可通过双击/拖入/右键创建
- [ ] 节点显示正确（标题/内容/端口）
- [ ] 图片/视频/音频节点支持文件上传
- [ ] 节点之间可连线（仅允许合法端口组合）
- [ ] 连线后数据正确传递（文本→提示词、图片→首帧等）
- [ ] Ctrl+G 打组成功
- [ ] Ctrl+Z 撤销、Ctrl+Shift+Z 重做正常
- [ ] 右键菜单功能完整
- [ ] 项目自动保存（30s，写入 SQLite）
- [ ] 深色主题统一（CSS Variables）
- [ ] 拖入文件可自动创建对应节点
- [ ] 日志系统正常输出（electron-log）
- [ ] Core 模块单元测试覆盖率 ≥ 80%
- [ ] 错误处理框架正常工作（统一异常捕获 + 用户友好提示）
