...
3. 从文本节点 `prompt` 拖线到 **图片节点** `prompt` → 选中图片节点 → 底部 **Generator Drawer** 打开 → 左侧大图预览、右侧选模型与风格 → 顶栏或面板内点「生成」
...
- **左侧预览** — 当前生成图；可拖拽调整高度与宽度（与画布节点尺寸无关）
- **风格 chip** — 即时预览生效 prompt；部分风格可一键应用推荐模型
- **顶栏「生成」** — Drawer 任意滚动位置可点
- **节点标题** — 双击画布节点标题可改名；新建时自动「图片 1」「图片 2」递增
